...
export default function Checkout() {
  ...
  const handlePay = async () => {
    if (paymentMethod === 'card') {
      ...
      if (rawNumber.length < 16) return setModal({ open: true, type: 'error', title: 'ข้อมูลไม่ครบ', msg: 'กรุณากรอกหมายเลขบัตร 16 หลัก' });
      if (!cardData.name.trim()) return setModal({ open: true, type: 'error', title: 'ข้อมูลไม่ครบ', msg: 'กรุณากรอกชื่อบนบัตร' });
      if (cardData.expiry.length < 5) return setModal({ open: true, type: 'error', title: 'ข้อมูลไม่ครบ', msg: 'กรุณากรอกวันหมดอายุ (MM/YY)' });
      if (cardData.cvv.length < 3) return setModal({ open: true, type: 'error', title: 'ข้อมูลไม่ครบ', msg: 'กรุณากรอก CVV 3 หลัก' });
    }

    if (!localStorage.getItem('token')) return setModal({ open: true, type: 'error', title: 'แจ้งเตือน', msg: 'กรุณาเข้าสู่ระบบก่อนซื้อตั๋ว' });
    ...
    try {
      if (tradeItem) {
        await fetchAPI(`/trades/${tradeItem.id}/buy`, { method: 'POST' });
      } else {
        await fetchAPI('/tickets/buy', {
          method: 'POST',
      ...
      setLoading(false);
      setStep('payment');
      setModal({ open: true, type: 'error', title: 'ผิดพลาด', msg: err.message || 'ระบบขัดข้อง กรุณาลองใหม่' });
    }
  };
          ...
          {step === 'payment' && (
               ...
               <div className="text-center py-6">
                 <div className="bg-white rounded-2xl p-6 inline-block mb-4">
                   <svg width="160" height="160" viewBox="0 0 160 160">
                     ...
                     {[0,1,2,3,4,5,6].map(r => [0,1,2,3,4,5,6].map(c => (((r < 3 || r > 3) && (c < 3 || c > 3)) || (r === 3 && c === 3) ? <rect key={`${r}-${c}`} x={10+r*20} y={10+c*20} width="18" height="18" fill={Math.random() > 0.5 ? '#000' : '#fff'} rx="2"/> : null)))}
                     ...
                     <rect x="22" y="22" width="34" height="34" fill="#000" rx="2"/><rect x="104" y="22" width="34" height="34" fill="#000" rx="2"/><rect x="22" y="104" width="34" height="34" fill="#000" rx="2"/>
                   </svg>
                 </div>
             ...
             {paymentMethod === 'truemoney' && (
               <div className="text-center py-6">
                 ...
                 <input type="tel" inputMode="numeric" placeholder="08x-xxx-xxxx" maxLength={10} className="w-full p-3 bg-gray-800 border border-gray-700 rounded-xl text-white text-center font-mono focus:border-yellow-500 outline-none mb-2" />
                 <p className="text-yellow-400 font-extrabold text-2xl mt-4">฿{total.toLocaleString()}</p>
               </div>
             )}

             <div className="flex gap-3 mt-6">
               <button onClick={() => setStep('summary')} className="flex-1 py-4 bg-gray-800 hover:bg-gray-700 text-white font-bold rounded-2xl transition">← กลับ</button>
               <button onClick={handlePay} className="flex-[2] py-4 bg-green-600 hover:bg-green-500 text-white font-extrabold rounded-2xl text-lg transition flex items-center justify-center gap-2"><Lock size={18} /> ยืนยันชำระเงิน ฿{total.toLocaleString()}</button>
             </div>
             <p className="text-center text-gray-600 text-xs mt-4 flex items-center justify-center gap-1"><Lock size={12} /> ข้อมูลการชำระเงินของคุณถูกเข้ารหัสด้วย SSL</p>
           </div>
          )}
        </div>
        ...
        <div className="lg:col-span-2">
          <div className="bg-gray-900 border border-gray-800 rounded-3xl p-6 sticky top-24">
            ...
            <div className="space-y-2 mb-4">
              {tradeItem ? (
                 <div className="flex justify-between text-sm"><span className="text-gray-400">{tradeItem.ticket.seat.tier} — แถว {tradeItem.ticket.seat.row} ที่ {tradeItem.ticket.seat.number}</span><span className="text-white">฿{tradeItem.price.toLocaleString()}</span></div>
              ) : (
                selectedSeats.map((seat: any) => (
                  <div key={seat.id} className="flex justify-between text-sm"><span className="text-gray-400">{seat.tier} — แถว {seat.row} ที่ {seat.number}</span><span className="text-white">฿{seat.price.toLocaleString()}</span></div>
                ))
              )}
            </div>
