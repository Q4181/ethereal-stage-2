...
export default function Checkout() {
  ...
  const total = selectedSeats.reduce((sum: number, s: any) => sum + s.price, 0);
  ...
  const handlePay = async () => {
    ...
    if (paymentMethod === 'card') {
      ...
      if (rawNumber.length < 16) {
        setModal({ open: true, type: 'error', title: 'ข้อมูลไม่ครบ', msg: 'กรุณากรอกหมายเลขบัตร 16 หลัก' });
        return;
      }
      if (!cardData.name.trim()) {
        setModal({ open: true, type: 'error', title: 'ข้อมูลไม่ครบ', msg: 'กรุณากรอกชื่อบนบัตร' });
        return;
      }
      if (cardData.expiry.length < 5) {
        setModal({ open: true, type: 'error', title: 'ข้อมูลไม่ครบ', msg: 'กรุณากรอกวันหมดอายุ (MM/YY)' });
        return;
      }
      if (cardData.cvv.length < 3) {
        setModal({ open: true, type: 'error', title: 'ข้อมูลไม่ครบ', msg: 'กรุณากรอก CVV 3 หลัก' });
        return;
      }
    }
    ...
    await new Promise(res => setTimeout(res, 2000)); // จำลอง delay
    ...
    try {
      ...
      if (data.success) {
        setStep('summary'); // reset step ก่อนแสดง modal
          ...
          {step === 'summary' && (
            <div className="bg-gray-900 border border-gray-800 rounded-3xl p-8">
              ...
              <button onClick={() => setStep('payment')}
                className="w-full bg-blue-600 hover:bg-blue-500 text-white font-extrabold py-4 rounded-2xl text-lg transition">
                ดำเนินการชำระเงิน →
              </button>
            </div>
          )}
        ...
        <div className="lg:col-span-2">
          <div className="bg-gray-900 border border-gray-800 rounded-3xl p-6 sticky top-24">
            ...
            <div className="mt-4 space-y-2 text-xs text-gray-600">
              <p className="flex items-center gap-1"><CheckCircle size={12} className="text-green-600" /> ไม่มีค่าธรรมเนียมแฝง</p>
              <p className="flex items-center gap-1"><CheckCircle size={12} className="text-green-600" /> ตั๋วส่งทันทีหลังชำระ</p>
              <p className="flex items-center gap-1"><CheckCircle size={12} className="text-green-600" /> ปลอดภัยด้วย SSL</p>
            </div>
          </div>
        </div>
      </div>
