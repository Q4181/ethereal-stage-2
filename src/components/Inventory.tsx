...
export default function Inventory() {
  ...
  const fetchAll = async () => {
    try {
      const [ticketData, tradeData] = await Promise.all([
        fetchAPI('/users/me/tickets'),
        fetchAPI('/users/me/trades')
      ]);
  ...
  const openTradeByTicketId = Object.fromEntries(myTrades.selling.filter(t => t.status === 'OPEN').map(t => [t.ticketId, t]));

  const handleListForSale = async (ticketId: number, maxPrice: number) => {
    const price = parseInt(sellPrice[ticketId] || '0');
    if (!price || price <= 0) return setModal({ open: true, type: 'error', title: 'ผิดพลาด', msg: 'กรุณากรอกราคาที่มากกว่า $0', action: null });
    if (price > maxPrice) return setModal({ open: true, type: 'error', title: 'ผิดพลาด', msg: `คุณสามารถตั้งราคาได้สูงสุด $${maxPrice} เท่านั้น`, action: null });
    
    try {
      await fetchAPI('/trades', { method: 'POST', body: JSON.stringify({ ticketId, price }) });
      ...
      setModal({ open: true, type: 'error', title: 'ผิดพลาด', msg: err.message || 'ระบบขัดข้อง', action: null });
    }
  };
  ...
  const handleCancelTrade = (tradeId: number) => {
    setModal({
      ...
      action: async () => {
        try {
          await fetchAPI(`/trades/${tradeId}`, { method: 'DELETE' });
          setModal({ open: true, type: 'success', title: 'ยกเลิกสำเร็จ', msg: 'ยกเลิกการลงขายเรียบร้อยแล้ว', action: null });
          fetchAll();
        } catch (err: any) { setModal({ open: true, type: 'error', title: 'ผิดพลาด', msg: err.message || 'ระบบขัดข้อง', action: null }); }
      }
    });
  };
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-12">
      <Modal isOpen={modal.open} type={modal.type} title={modal.title} message={modal.msg} onClose={() => setModal({ ...modal, open: false })} onConfirm={modal.type === 'confirm' && modal.action ? modal.action : undefined} />
            ...
            return (
              <div key={ticket.id} className="bg-gradient-to-br from-blue-900/40 to-gray-900 border border-blue-500/20 rounded-2xl p-6 relative overflow-hidden shadow-xl">
                ...
                <div className="absolute top-4 right-4 z-10 flex flex-col gap-2 items-end">
                  ...
                  {isUntradeable ? (
                    <div className="flex items-center gap-1 bg-red-500/20 text-red-400 border border-red-500/30 text-xs px-2 py-1 rounded-full font-bold backdrop-blur-md"><ShieldAlert size={12} /> UNTRADEABLE</div>
                  ) : (
                    <div className="flex items-center gap-1 bg-green-500/20 text-green-400 border border-green-500/30 text-xs px-2 py-1 rounded-full font-bold backdrop-blur-md"><ShieldCheck size={12} /> TRADEABLE</div>
                  )}
                </div>
